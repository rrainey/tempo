...
void BinaryLogger::writeNmeaSentence(const char *pSentence) {
    if (getOperatingState() == OperatingState::Running) {
        ...
        if (len > 0) {
            ...
            logfile.write((uint8_t *)&record, len + TEMPO_RAW_FILE_HEADER_SIZE);
        }
    }
}
    ...
    if (nmea.isValid()) {
        ...
        if (getOperatingState() == OperatingState::Running &&
            ...
            logfile.timestamp(T_CREATE, nmea.getYear(), nmea.getMonth(),
                              ...
                              nmea.getSecond());
            logfile.timestamp(T_ACCESS, nmea.getYear(), nmea.getMonth(),
                              nmea.getDay(), nmea.getHour(), nmea.getMinute(),
                              nmea.getSecond());
            logfile.timestamp(T_WRITE, nmea.getYear(), nmea.getMonth(),
                              nmea.getDay(), nmea.getHour(), nmea.getMinute(),
                              nmea.getSecond());
            logfileDateSet = true;
        }
